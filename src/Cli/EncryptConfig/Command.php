...
final readonly class Command
{
	...
	public function __invoke(Input $input, OutputInterface $output): int
	{
		...
		$config = $this->cryptoHelper->encryptConfig($config);

		$outputContent = $this->cryptoHelper->renderConfig(
			$config,
			$this->cryptoHelper->getDefaultEncryptedFileHeader($input->outputFile),
		);

		file_put_contents($input->outputFile, $outputContent);

		$output->writeln('Wrote encrypted config to ' . $input->outputFile);
